...
<div class="card" >
    <div class="card-header">
        <h3 class="card-title "> National Strategies NFIS List</h3>
        <div class="card-tools">
            @if ($add_permission)
                <a href="{{ route('national_strategies_nfis.create') }}" type="button" class="btn btn-primary">
                    <i class="fa fa-plus"></i>
                    New National Strategies NFIS
                </a>
            @endif
        </div>
        <!-- /.card-tools -->
    ...
    <script>
        $(function() {
            $('#list').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: '{{ route('national_strategies_nfis.list') }}',
                    method: 'post',
                    data: function(d) {
