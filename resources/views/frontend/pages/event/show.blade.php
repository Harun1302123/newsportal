        ...
        <div class="nfis-breadcrumb">
            <div class="container">
                <ul class="nfis-breadcrumb-lists">
                    ...
                    <li class="nfis-bc-item active">{{ languageStatus() == 'en' ? 'Event Details' : "অনুষ্ঠানের বিবরণ" }}</li>
                </ul>
            </div>
        </div>
                        ...
                        <div class="col-lg-8">
                            <div class="nfis-post-details">
                                <div class="nfis-post-title">
                                    <h3>{{ languageStatus() == 'en' ? ($event->heading_en??null) : ($event->heading_bn??null) }}</h3>
                                    <span class="date-text">{{ !empty($event->updated_at) ? languageStatus() == 'bn' ? App\Libraries\CommonFunction::convertEnglishDateToBangla(date('d M, Y', strtotime($event->updated_at))) : date('d M, Y', strtotime($event->updated_at)) : null }}</span>


                                </div>
                                <div class="single-photo-item">
                                    <img src="{{ asset(CommonFunction::setImageOrDefault($event->image??null)) }}" alt="Images">
                                </div>
                                <p>{!! languageStatus() == 'en' ? $event->details_en??null : $event->details_bn??null !!}</p>
                        ...
                        <div class="col-lg-4">
                            <div class="nfis-post-sidebar">
                                <div class="widget-item">
                                    <h3> {{ languageStatus() == 'en' ? 'Recent Events' : "সম্প্রতিক ঘটনাবলী" }}</h3>
                                    @foreach($recent as $single_recent)
                                    <a href="{{ route('frontend.event.show',['id' => Encryption::encodeId($single_recent->id)]) }}" class="post-lists-item">
                                        <div class="post-list-img"  style="background-image: url({{ asset(CommonFunction::setImageOrDefault($single_recent->image??null)) }});" ></div>
                                        <div class="post-list-desc">
                                            <h4>{{ languageStatus() == 'en' ? Str::limit($single_recent->heading_en??null, 80) : Str::limit($single_recent->heading_bn??null, 80) }}</h4>
                                            <span class="date-text">{{ !empty($single_recent->updated_at) ? languageStatus() == 'bn' ? App\Libraries\CommonFunction::convertEnglishDateToBangla(date('d M, Y', strtotime($single_recent->updated_at))) : date('d M, Y', strtotime($single_recent->updated_at)) : null }}</span>
                                ...
                                <div class="widget-item">
                                    <h3>{{ languageStatus() == 'en' ? 'Related Events' : "সম্পর্কিত ঘটনাবলী" }}</h3>
                                    @foreach($related as $single_related)
                                    <a href="{{route('frontend.event.show', ['id' => Encryption::encodeId($single_related->id)])}}" class="post-lists-item">
                                        <div class="post-list-img" style="background-image: url('{{ asset(CommonFunction::setImageOrDefault($single_related->image??null)) }}')"></div>
                                        <div class="post-list-desc">
                                            <h4>{{ languageStatus() == 'en' ? Str::limit($single_related->heading_en??null, 80) : Str::limit($single_related->heading_bn??null, 80) }}</h4>
