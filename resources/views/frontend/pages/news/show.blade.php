        ...
        <div class="nfis-breadcrumb">
            <div class="container">
                <ul class="nfis-breadcrumb-lists">
                    ...
                    <li class="nfis-bc-item active">{{ languageStatus() == 'en' ? 'News Details' : "খবর বিস্তারিত" }}</li>
                </ul>
            </div>
        </div>
                        ...
                        <div class="col-lg-8">
                            <div class="nfis-post-details">
                                <div class="nfis-post-title">
                                    <h3>{{ languageStatus() == 'en' ? ($article->title_en??null) : ($article->title_bn??null) }}</h3>

                                    <span class="date-text">{{ !empty($article->updated_at) ? languageStatus() == 'bn' ? App\Libraries\CommonFunction::convertEnglishDateToBangla(date('d M, Y', strtotime($article->updated_at))) : date('d M, Y', strtotime($article->updated_at)) : null }}</span>
                                </div>
                                <div class="single-photo-item">
                                    <img src="{{ asset(CommonFunction::setImageOrDefault($article->image??null)) }}" alt="Images">
                                </div>
                                <p>{!! languageStatus() == 'en' ? $article->content_en??null : $article->content_bn??null !!}</p>
                        ...
                        <div class="col-lg-4">
                            <div class="nfis-post-sidebar">
                                <div class="widget-item">
                                    <h3>{!! languageStatus() == 'en' ? "Recent News" : "সাম্প্রতিক খবর" !!}</h3>
                                    @foreach($recent as $single_recent)
                                    <a href="{{ route('frontend.news.show',['id' => Encryption::encodeId($single_recent->id)]) }}" class="post-lists-item">
                                        ...
                                        <div class="post-list-desc">
                                            <h4>{{ languageStatus() == 'en' ? Str::limit($single_recent->title_en??null, 80) : Str::limit($single_recent->title_bn??null, 80) }}</h4>
                                ...
                                <div class="widget-item">
                                    <h3>{!! languageStatus() == 'en' ? "Related News" : "সম্পর্কিত খবর" !!}</h3>
                                    @foreach($related as $single_related)
                                    <a href="{{route('frontend.news.show', ['id' => Encryption::encodeId($single_related->id)])}}" class="post-lists-item">
